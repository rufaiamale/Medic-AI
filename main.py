...
# Symptoms dictionary (keep only 132 keys matching model)
symptoms_dict = {
    ...
    'spotting_urination': 13, 'fatigue': 14, 'weight_gain': 15, 'anxiety': 16, 'cold_hands_and_feets': 17,
    ...
    'continuous_feel_of_urine': 91, 'passage_of_gases': 92, 'internal_itching': 93, 'toxic_look_typhos': 94,
    ...
    'history_of_alcohol_consumption': 116, 'blood_in_sputum': 117, 'prominent_veins_on_calf': 118, 'palpitations': 119,
    'painful_walking': 120, 'pus_filled_pimples': 121, 'blackheads': 122, 'scurring': 123, 'skin_peeling': 124,
    'silver_like_dusting': 125, 'small_dents_in_nails': 126, 'inflammatory_nails': 127, 'blister': 128,
    'red_sore_around_nose': 129, 'yellow_crust_ooze': 130, 'extra_symptom_removed': 131  # remove this if extra
}
...
def get_predicted_value(patient_symptoms):
    # Ensure exact 132 features
    features_list = list(symptoms_dict.keys())[:132]
    input_vector = np.zeros(len(features_list))
    for item in patient_symptoms:
        if item in features_list:
            input_vector[features_list.index(item)] = 1
...
@app.route("/")
def index():
    return render_template("index.html", symptoms_list=list(symptoms_dict.keys())[:132])
...
def predict():
    ...
    if not symptoms:
        message = "Please select or write symptoms correctly"
        return render_template('index.html', symptoms_list=list(symptoms_dict.keys())[:132], message=message)
    ...
    return render_template('index.html', predicted_disease=predicted_disease, dis_des=dis_des,
                           ...
                           workout=workout, symptoms_list=list(symptoms_dict.keys())[:132])
